...
func HashPassword(password string) (string, string, error) {
	...
	if err != nil {
		log.Err(err).Msgf("There was an error encrypting the password: %v", err.Error())
		return "", "", err
	}
...
func generateSalt() []byte {
	...
	if err != nil {
		log.Err(err).Msgf("There was an error generating the salt: %v", err.Error())
	}
...
func decodeHashFromString(hash string) ([]byte, error) {
	...
	if err != nil {
		log.Err(err).Msgf("There was an error decoding the password: %v", err.Error())
		return nil, err
	}
